...
class DashboardSafetyTests(unittest.TestCase):
    ...
    def test_phase_four_states_and_views_are_present(self) -> None:
        ...
        for identifier in (
            ...
            "fleet-output-tps-coverage", "performance-view", "model-ttft-p50",
            "model-ttft-p95", "model-input-p50", "model-input-p95",
            "model-call-tps-p50", "server-output-tps", "decode-concurrency-bands",
            "capacity-signals",
            "tool-observation",
        ):
        ...
        self.assertIn("decode_concurrency_bands", script)
        self.assertIn("generation_tokens_per_second", script)
        self.assertIn("gpu_kv_cache_usage_ratio", script)
        ...
        self.assertIn("Observed tools", page)
        self.assertIn("tool_observation_mode", script)
